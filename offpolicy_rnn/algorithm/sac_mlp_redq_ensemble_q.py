...
class SAC_MLP_REDQ_EnsembleQ(SAC):
    ...
    def _target_Q(self, state, last_state, action, last_action, next_state, done, reward, reward_input, alpha_detach):
        with torch.no_grad():
            ...
            next_target_Qs = [target_q_net.forward(next_state, state, action, next_act_sample, None, reward)[0] for
                              target_q_net in self.target_values]
            # min_i next_Q_i
            sampled_target_value_nets = np.random.permutation(next_target_Qs[0].shape[0])[:self.parameter.redq_m]
            next_target_Qs[0] = next_target_Qs[0][sampled_target_value_nets, ...]
    ...
    def _Q_loss(self, state, last_state, last_action, action, reward_input, target_Q):
        ...
        Q_loss_list = [(q_item - target_Q.unsqueeze(0)).pow(2).sum(dim=0).mean() for q_item in Qs]
    ...
    def _target_Q_discrete(self, state, last_state, action, last_action, next_state, done, reward, reward_input, alpha_detach):
        with torch.no_grad():
            ...
            _, _, next_act_sample, next_log_prob, _, _ = self.policy.forward(next_state, state, action_one_hot,
                                                                             None, reward)
            next_target_Qs = [
                target_q_net.forward(next_state, state, action_one_hot, next_act_sample, None, reward)[0] for
                target_q_net in self.target_values]
            # min_i next_Q_i
            min_next_target_Q = (next_target_Qs[0].min(dim=0, keepdim=False) - alpha_detach * next_log_prob) * next_log_prob.exp()
    ...
    def _Q_loss_discrete(self, state, last_state, last_action, action, reward_input, target_Q):
        ...
        Q_loss_list = [(q_item - target_Q.unsqueeze(0)).pow(2).mean() for q_item in Qs]
    ...
    def train_one_batch(self) -> Dict:
        ...
        for utd_idx in range(self.parameter.utd):
            ...
            state, last_state, action, last_action, next_state, done, reward, reward_input, timeout = map(
            ...
            state, last_state, action, last_action, next_state, done, reward, reward_input, timeout = map(lambda x: n2t(x, self.device),
                                                                                              [
                                                                                                  ...
                                                                                                  last_action, next_state,
                                                                                                  done, reward,
                                                                                                  reward_input, timeout
                                                                                              ])
            done[timeout > 0] = 0
            ...
            if (utd_idx + 1) / self.parameter.utd * self.parameter.policy_utd > policy_update_cnt:
                ...
                if self.parameter.policy_max_gradnorm is not None:
                    pi_grad_norm = torch.nn.utils.clip_grad_norm_(self.policy.parameters(),
                                                                  self.parameter.policy_max_gradnorm, norm_type=2)
