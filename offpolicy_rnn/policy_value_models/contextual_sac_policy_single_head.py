...
class ContextualSACPolicySingleHead(ContextualModel):
    MAX_LOG_STD = 2.0
    MIN_LOG_STD = -20.0
    def __init__(self, state_dim, action_dim, embedding_size, embedding_hidden, embedding_activations,
                 ...
                 reward_input=False, last_action_input=True, last_state_input=False, separate_encoder=False, output_logstd=True, name='ContextualSACPolicy'):
        ...
        super(ContextualSACPolicySingleHead, self).__init__(embedding_input_size=cum_dim,
    ...
    def forward(self, state: torch.Tensor, lst_state: torch.Tensor, lst_action: torch.Tensor, rnn_memory: Optional[RNNHidden], reward: Optional[torch.Tensor]=None, detach_embedding: bool=False) -> Tuple[
        ...
        logstd, logit = model_output.chunk(2, dim=-1)
