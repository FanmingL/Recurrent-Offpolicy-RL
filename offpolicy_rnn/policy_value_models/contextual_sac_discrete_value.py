...
class ContextualSACDiscreteValue(ContextualModel):
    def __init__(self, state_dim, action_dim, embedding_size, embedding_hidden, embedding_activations,
        ...
        super(ContextualSACDiscreteValue, self).__init__(embedding_input_size=cum_dim,
    ...
    def state_encoding(self, state):
        if self.separate_encoder:
            return self.uni_model_input_mapping_activation_func(self.state_input_encoder(state))
        else:
            return self.state_input_encoder(state)

    def forward(self, state: torch.Tensor, lst_state: torch.Tensor, lst_action: torch.Tensor,
                action: torch.Tensor, rnn_memory: Optional[RNNHidden], reward: Optional[torch.Tensor], detach_embedding: bool=False) -> Tuple[
        ...
        value, rnn_memory, embedding_output, full_rnn_memory = \
            self.meta_forward(embedding_input, self.state_encoding(state), rnn_memory, detach_embedding)
