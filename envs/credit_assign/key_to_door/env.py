...
class PycolabEnvironment(object):
    ...
    def __init__(
        ...
        if not has_tf:
            # TODO: there is an bug here, user must install tensorflow
            def apply_to_nested(structure, func):
                if isinstance(structure, (list, tuple)):
                    return type(structure)(apply_to_nested(sub_structure, func) for sub_structure in structure)
                else:
                    return func(structure)

            colours = apply_to_nested(self._game.colours, lambda c: float(c) * 255 / 1000)
        else:
