import numpy as np
import gym
from . import env, key_to_door
from . import env, visual_match
...
class KeyToDoor(gym.Env):
    def __init__(
        ...
        if continuous_act_space:
            self.action_space = gym.spaces.Box(np.array([-1.0] * 4), np.array([1.0] * 4), dtype=np.float32)
        else:
            self.action_space = gym.spaces.Discrete(4)  # 4 directions
        self.one_hot_actions = one_hot_actions
    ...
    def step(self, action):
        ...
        if self.continuous_act_space:
            action = np.argmax(np.reshape(action, (-1,)))
            # action = int(((action + 1) * 0.5) // 0.25)
            # if action >= 4:
            #     action = 3
        obs, r = self.pycolab_env.step(action)
        self._ret += r
        ...
        # print(f'info: {info}')
        return self._convert_obs(obs) / 255, r, done, info

    def reset(self):
        obs, _ = self.pycolab_env.reset()
        self._ret = 0.0

        return self._convert_obs(obs) / 255
