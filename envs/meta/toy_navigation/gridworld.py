...
class GridNavi(gym.Env):
    def __init__(
        ...
        **kwargs
        ...
        self.goals = self.np_random.sample(self.possible_goals, self.num_tasks)
        self.reset_task(0)
    ...
    def reset_model(self):
        ...
        if self.modify_init_state_dist:
            self._state = np.array(self.np_random.choice(self.states))  # For data collection
            while (self._state == self._goal).all():  # do not start in goal
                self._state = np.array(self.np_random.choice(self.states))
        else:
            self._state = np.array(self.np_random.choice(self.starting_states))
        self._belief_state = self._reset_belief()
        return self.get_obs()
